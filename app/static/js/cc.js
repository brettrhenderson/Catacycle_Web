...
function submitForm(form_url, responseHandler) {
    var formData = new FormData(document.getElementById('cc-form'))
    ...
    $.ajax(
    {
        ...
        error: function(jqXHR, textStatus, errorThrown)
            {
                // if fails
                alert('Continuous calibration failed with the following error: ' + jqXHR.responseText);
            }
    });
}
...
function submitCCHandler(form) {
    ...
    if (downloadImage) {
        ...
        submitForm('/cc', function (response) {
            document.getElementById('cc-result').src = response.data[0];
...
function submitDownloadData() {
    // get all of the information in the input cc form
    var cc_data = cloneWithSelects($('#cc-form')).find(':input')
    cc_data.attr('hidden', true);
    $('#download-form').append(cc_data);
